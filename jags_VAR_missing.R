...
# Lots of the y values are missing
...
# Pick which ones are missing
prop_missing <- 0.2
miss <- matrix(rbinom(T * k, 1, prob = prop_missing),
  nrow = T, ncol = k
)
# Also make sure that one of the first row is missing - just for checking
y[1, 1] <- NA
y[which(miss == 1, arr.ind = TRUE)] <- NA
...
model
{
  # Likelihood
  for (i in 1:k) {
    y[1,i] ~ dmnorm.vcov(A[i], Sigma[i,i])
  }
